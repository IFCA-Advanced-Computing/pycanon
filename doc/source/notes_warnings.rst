...
.. note::
    ...
    * Calculate the different properties for each sensitive attribute individually, and take the maximum or minimum as appropriate (for example, the minumum value of :math:`\ell` for :math:`\ell`-diversity and the maximum value for :math:`\alpha` in the case of (:math:`\alpha`,k)-anonymity).
...
.. warning::
    It's important to take into account that the values of *t* and :math:`\delta` for t-closeness and :math:`\delta`-disclosure privacy respectively must be strictly greater than the ones obtained using *pyCANON* (see the definition of that techniques).
